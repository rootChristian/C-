        ...
        protected override void Seed(AnagraficaIcpContext context)
        {
            try
            {
                ...
                */
                ...
                /*
                using (System.IO.StreamReader reader = new StreamReader(fileIcp))
                {
                    using (var readerCsv = new CsvReader((IParser)reader))
                    {
                        var L = readerCsv.GetRecords<Icp>();
                        foreach (var item in L)
                        {
                        ...
                        //icp.ForEach(s => context.Icp.Add(s));
                        //context.SaveChanges();
                    }
                }
                
                using (System.IO.StreamReader reader = new StreamReader(fileAnagrafica))
                {
                    using (var readerCsv = new CsvReader((IParser)reader))
                    {
                        var L = readerCsv.GetRecords<Anagrafica>();
                        ...
                        //anagrafica.ForEach(s => context.Anagrafica.Add(s));
                        //context.SaveChanges();
                    }
                }
                */
